...
- Access to the GCP project (e.g., `biodiversidad-taller`)
...
```bash
gcloud projects add-iam-policy-binding "biodiversidad-taller" \
  --member="serviceAccount:github-actions-rle@biodiversidad-taller.iam.gserviceaccount.com" \
...
```bash
gcloud projects describe biodiversidad-taller --format="value(projectNumber)"
```
...
gcloud iam service-accounts add-iam-policy-binding \
  "github-actions-rle@biodiversidad-taller.iam.gserviceaccount.com" \
  --project="biodiversidad-taller" \
...
| `GCP_SERVICE_ACCOUNT` | `github-actions-rle@biodiversidad-taller.iam.gserviceaccount.com` |
...
```bash
python scripts/rasterize_ecosystem_to_cog.py --project biodiversidad-taller
```
...
- **enables the `storage.googleapis.com` API** and **creates a public-read bucket** `gs://biodiversidad-taller-rle-cogs` if it does not already exist;
...
```
Caller does not have required permission to use project biodiversidad-taller.
